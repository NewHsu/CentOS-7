...
## 1. LVM介绍及原理
### 1.1 LVM介绍
* 企业中所有服务器的使用空间都会因为业务的需求增长而不断扩展，传统的磁盘分区使用固定大小的物理分区，如果需要重新调整将会非常麻烦，无法适应数据高速增长的扩缩容需求。
* 现在的Linux系统使用着一个非常好用的磁盘系统管理工具，名为“逻辑卷管理（LVM）”。
* 逻辑卷管理（LVM）可以创建和管理逻辑卷，但是不能直接管理和使用磁盘。可以弹性的管理卷的扩缩容，并且对数据有绝对的保障。
* 逻辑卷可以用于单个物理磁盘分区、SAN、RAID等都可以被加入逻辑卷组中，然后在池化的卷组中，进行空间切分，并创建逻辑卷，以供使用。
* 如果某个磁盘启动失败，利用卷组将用于替换的磁盘转换成物理卷，并且逻辑卷的扩展功能，可以将数据迁移到新的磁盘而不会丢失任何数据。

![](../images/LVM/1.png)
>图1-1展示LVM整体结构

### 1.2 LVM基本术语
* LVM实际是磁盘分区和文件系统之间的一个逻辑层，对使用者屏蔽下层磁盘资源，提供一个抽象的卷组池，在卷组的LV卷上建立逻辑文件系统。
* 在深入学习之前，先行熟悉以下几个比较重要的LVM术语：
...
2. 物理卷（physical volume）
        ...
        物理卷就是指硬盘分区或从逻辑上与磁盘分区具有同样功能的设备，是LVM的基本存储逻辑块，但和基本的物理存储介质（如分区、磁盘等）比较，却包含有与LVM相关的管理参数。

3. 卷组（Volume Group）

        LVM卷组类似于非LVM系统中的物理硬盘，由物理卷组成，可以在卷组上创建一个或多个“LVM分区”（逻辑卷），LVM卷组由一个或多个物理卷组成。

4. 逻辑卷（logicalvolume）

        LVM的逻辑卷类似于系统中的硬盘分区，在逻辑卷之上可以建立文件系统。
...
* 依据上图来看，最底层依然是物理磁盘设备，然后通过PV化将底层的物理磁盘和分区整合到一个VG卷组当中，在逻辑上形成一个较大的存储资源池，在VG中通过LV的形式进行输出。
* 物理卷PV化后，由大小等同的基本单元PE组成，在加入VG卷组，创建LV后，由大小等同的基本单元LE组成，PE和LE有着一一对应的关系。
* 由上图可以看出，一个卷组由一个或多个物理卷组成，物理卷中的PE，可以分别对应多个LV，与LE形成对应关系。
* LV逻辑卷之上，可以通过mkfs等指令在其上创建文件系统。
...
## 总结
本篇为LVM基础操作，其中创建和扩容，是生产环境中应用最多的，至于所谓的缩容需求，一定要谨慎再谨慎，当心数据。
在下一篇中将进行LVM的Cache和Snapshot的讲解。
