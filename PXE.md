...
KickStart是一种无人职守安装方式。KickStart的工作原理是通过记录安装过程中所需人工干预填写的各种参数，并生成一个名为ks.cfg的文件；在后续的无人值守安装过程中，如果出现要求填写参数的情况时，安装程序会首先去查找KickStart的文件，匹配到合适的参数时，就采用该参数，如果没有匹配到，责需安装者手工干预。所以，在PXE安装过程中，只需要告诉安装程序从何处取ks.cfg文件，然后默默的等安装完毕，安装程序会根据ks.cfg中设置参数、所需执行的脚本来安装和部署系统，并结束安装。
