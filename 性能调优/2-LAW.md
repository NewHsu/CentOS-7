...
## 运用实例掌握队列理论
### 实例1：
* 一个新管理员在2台不同的服务器进行磁盘读写测试，一台服务器配置较好，另外一台比较low，但是他得到的结果却是读写速度基本一样，测试的方法是：

  # dd if=/dev/zero of=/dev/null

* 2台主机资源配比差异较大，结果却相同？问题出在哪？

* 其实这是一个很有意思的测试，因为/dev/zero数据是可以被无限读取的，而/dev/null中的数据永远被丢弃。那么问题来了，这总神一般的操作，磁盘根本就没有参与，而且这种测试方法无法体现出多CPU的优势，因此测试出来的结果基本相等。

### 实例2：
* 在硬件资源紧张的情况下，你的主管要求你在某个服务器上部署一个应用，但是该应用属于磁盘读写密集型的应用，在通过iostat查看该程序运行情况的时候发现，该磁盘上每秒钟大概有290个读清求，而写请求是0个，平均服务时间是3.36毫秒。那么如何对你的主管解释这种情况：
...
* 可以测出磁盘使用率为0.9744，根据公式：磁盘利用率 ＝ 服务时间 * 到达率
...
* 或者换句话说：最大到达律（max arrival rate） = 1 / 服务时间（service time）。
